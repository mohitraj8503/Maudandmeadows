/*
 Image optimization script (CommonJS) for projects with "type":"module"
*/
const fs = require('fs');
...
async function run() {
  ...
  // write a simple manifest of available bases for runtime checks
  try {
    const outFiles = fs.readdirSync(OUT_DIR);
    const bases = new Set();
    outFiles.forEach((fn) => {
      const m = fn.match(/^(.*)-\d+\.(avif|webp|jpg)$/i);
      if (m) bases.add(m[1]);
    });
    const manifest = { bases: Array.from(bases) };
    fs.writeFileSync(path.join(OUT_DIR, 'manifest.json'), JSON.stringify(manifest, null, 2));
    console.log('Wrote manifest.json with', manifest.bases.length, 'entries');
  } catch (err) {
